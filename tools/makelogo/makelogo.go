...
func genLogoFile(img image.Image, transColor color.RGBA, logoVar string) (string, error) {
	var (
		buf         bytes.Buffer
		bounds      = img.Bounds()
...
Height: %d,
TransparentIndex: 0,
`, logoVarName, bounds.Size().X, bounds.Size().Y)
	...
	// Footer
	fmt.Fprint(&buf, "}\n)\n")
	fmt.Fprintf(&buf, "func init(){ ConsoleLogo = &%s }\n", logoVarName)
